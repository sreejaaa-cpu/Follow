...
function Login(props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await axios.post("http://localhost:1010/user/login", { email, password },{withCredentials: true})
      .then((response) => {
    ...
    <div className="max-w-md mx-auto mt-16 p-8 border rounded-lg shadow-lg bg-white">
      ...
      <div className="mt-4 text-center text-gray-700">
        <p>
          ...
          <span onClick={props.x} className="text-red-600 cursor-pointer font-medium hover:underline"> Sign up</span>
        </p>
      </div>
    </div>
  );
